...
fun predicate p [] = Failure
  | predicate p (x::xs) = if p x then pure x xs else Failure
fun not p [] = Failure
  | not p (x::xs) = if p x then Failure else pure x xs
...
fun andp f g xs = case g xs of
		    ...
		    | Success _ => f xs
fun f <&&> g = andp f g
fun some f x = (plus (cons f (some f)) (map (fn x => [x]) f)) x
fun many f = plus (some f) (pure [])
